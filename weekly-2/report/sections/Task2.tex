...
\begin{lstlisting}
  ...
  let (_, flags') = mkFlagArray Si (iota (length Si))
...
A bounds check has been added, which makes it such that all out of bounds
indices in the original scatter is mapped to $-1$\footnote{-1 is used, but any
out of bounds index could be used, it does not matter.}, and only the in bounds
indices is taken into account when calculating the flat index. 
