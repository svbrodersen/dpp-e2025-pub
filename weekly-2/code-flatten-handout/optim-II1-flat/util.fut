...
def mkFlagArray 't [m]
                (aoa_shp: [m]u32)
                (zero: t)
                -- aoa_shp=[0,3,1,0,4,2,0]
                (aoa_val: [m]t) : ([m]u32, []t) =
  -- aoa_val=[1,1,1,1,1,1,1]
  let shp_rot =
    map (\i ->
           if i == 0
           then 0
           else -- shp_rot=[0,0,3,1,0,4,2]
                aoa_shp[i - 1])
        (iota m)
  let shp_scn = scan (+) 0 shp_rot
  -- shp_scn=[0,0,3,4,4,8,10]
  let aoa_len =
    if m == 0
    then 0i64
    else --aoa_len = 10
...
def binSearch [n] (as: [n]u32) (x: u32) : u32 =
  #[unsafe]
  let n = assert (n > 0) n
  in if x >= last as
     then u32.i64 (n - 1)
          ...
          in if l != h
             then h
             else let ma = #[unsafe] as[i64.u32 l]
                  in loop m = h while m < u32.i64 n - 1 && #[unsafe] as[i64.u32 m + 1] == ma do m + 1
