...
def optimII1Ker [m][n][q] (Sa: [m]u32, Da: [n]f32)
  ...
  let (ba1, flag_array_a') = mkFlagArray Sa 0 (iota (length Sa))
  let flag_array_a = map bool.u32 flag_array_a' 
  let iia1 = sgmScan (+) 0 flag_array_a flag_array_a'
  let iia2 = map2(\i sgm -> i - ba1[sgm]) (iota (length flag_array_a)) iia1 
  let (bb1, flag_array_b') = mkFlagArray Sb 0 (iota (length Sb))
  let flag_array_b = map bool.u32 flag_array_b' 
  let iib1 = sgmScan (+) 0 flag_array_b flag_array_b'
  let iib2 = map2(\i sgm -> i - bb1[sgm]) (iota (length flag_array_b)) iib1 
